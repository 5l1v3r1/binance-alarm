...
def get_alarm_count():
    ...
    for cell in alarm_db['alarm_count']:
        ...
        if cell != 0:
            ...
            for i in alarm_list:
                if alarm_db[alarm_db['symbol'] == symbol].to_dict('records')[0][i] != 0:
                    alarms.append(alarm_db[alarm_db['symbol'] == symbol].to_dict('records')[0][i])
            # print(get_live_price(symbol), f"{symbol}: {alarms} alarm count: {cell}")
            print(f"{symbol}: {alarms} alarm count: {cell}")
